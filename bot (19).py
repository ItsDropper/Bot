...
# Turso is a hosted, SQLite-compatible database (libSQL). The wrapper classes
# below let every existing "with get_db() as conn: conn.execute(...).fetchone()"
# call site in this file keep working completely unchanged — the SQL is the
# same SQLite dialect, and rows still support dict-style ["column"] access.
# The data now lives permanently in Turso, so there's no more copying tiers.db
# to/from GitHub, and no more losing data on redeploy.

class _ResultWrapper:
    def __init__(self, rs):
        self._rows = [dict(zip(rs.columns, row)) for row in rs.rows]
...
class _ConnWrapper:
    ...
    def execute(self, sql, params=()):
        ...
        return _ResultWrapper(rs)

    def commit(self):
        # Each statement is applied immediately over the libSQL HTTP protocol,
        # so there's nothing to flush — this is only here so existing
        # "conn.commit()" call sites don't need to change.
        pass
...
def get_db():
    ...
    return _ConnWrapper(_turso_client)
