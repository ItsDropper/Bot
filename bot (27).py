...
SUPPORT_ROLE_ID = 1514274160329756673
TICKET_LOG_CHANNEL_ID = 1514004581410013264

intents = discord.Intents.default()

# ── Database (Supabase / Postgres via psycopg2) ───────────────────────────────
# psycopg2 is synchronous, so every DB call runs in a thread via run_in_executor
# to avoid blocking the async Discord event loop.
...
def _db_raw(sql, params=()):
    """Execute SQL synchronously, return (rows, description)."""
    conn = _get_conn()
...
class _ConnCtx:
    """Synchronous context manager so all existing `with get_db() as conn:` call sites work unchanged."""
    def __enter__(self):
        return self
    ...
    def execute(self, sql, params=()):
        return _DbResult(_db_raw(sql, params))

    def commit(self):
        pass  # _db_raw auto-commits after every statement

def get_db():
    return _ConnCtx()

async def db(sql, params=(), fetch="none"):
    """Async helper — runs _db_raw() in a thread."""
    loop = asyncio.get_event_loop()
    rows = await loop.run_in_executor(None, lambda: _db_raw(sql, params))
    if fetch == "one":
...
async def insert_tier_record(user_id: str, gamemode: str, tier: str, tester_id: str, notes: str = None):
...
async def get_cooldown_expiry(discord_id: str, gamemode: str):
    row = await db(
        "SELECT timestamp FROM tier_history WHERE user_id = ? AND gamemode = ?"
        " ORDER BY timestamp DESC LIMIT 1",
        (discord_id, gamemode), fetch="one"
    )
    if not row:
        return None
...
def support_only():
    async def predicate(interaction: discord.Interaction):
        ...
        has_role = support_role and support_role in interaction.user.roles
        is_admin = interaction.user.guild_permissions.administrator
        if has_role or is_admin:
            return True
        await interaction.response.send_message(
            "You need the **support** role to use this command.", ephemeral=True
        )
        return False
    return app_commands.check(predicate)
...
class TicketModal(discord.ui.Modal, title="Tier Test Application"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        expiry = await get_cooldown_expiry(str(interaction.user.id), self.gamemode)
        if expiry:
        ...
        embed = discord.Embed(title="🎟️ Tier Test Ticket", color=discord.Color.green())
...
class TierTestView(discord.ui.View):
    ...
    async def check_cooldown(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        lines = []
        for gm in GAMEMODES:
            expiry = await get_cooldown_expiry(discord_id, gm)
            if expiry:
                ts = int(expiry.timestamp())
...
class SupportTicketModal(discord.ui.Modal, title="Create Support Ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Anti-duplicate: block if user already has an open support ticket
        if category:
            for ch in category.channels:
                if ch.name.startswith("support-"):
                    ow = ch.overwrites_for(interaction.user)
                    if ow.view_channel is True:
                        await interaction.response.send_message(
                            f"You already have an open support ticket: {ch.mention}", ephemeral=True
                        )
                        return
        ...
        ping = support_role.mention if support_role else "@support"
        await channel.send(content=ping, embed=embed, view=CloseTicketView())
        await interaction.response.send_message(
            f"Ticket created: {channel.mention}", ephemeral=True
        )
...
class TesterApplicationModal(discord.ui.Modal, title="Tester Application"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not channel:
            ...
            return
        embed = discord.Embed(title="⚔️ New Tester Application", color=0x7B2FBE)
        embed.set_thumbnail(url=interaction.user.display_avatar.url)
...
class StaffApplicationModal(discord.ui.Modal, title="Staff Application"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not channel:
            await interaction.response.send_message("Application channel not found. Contact an admin.", ephemeral=True)
            return
        embed = discord.Embed(title="🛡️ New Staff Application", color=0xFFAA00)
        embed.set_thumbnail(url=interaction.user.display_avatar.url)
        embed.add_field(name="Applicant", value=f"{interaction.user.mention} (`{interaction.user.name}`)", inline=False)
...
class ApplicationPanelView(discord.ui.View):
    ...
    @discord.ui.button(label="Apply — Tester", style=discord.ButtonStyle.primary, custom_id="app_open_tester", emoji="⚔️")
    async def apply_tester(self, interaction: discord.Interaction, button: discord.ui.Button):
        warning_embed = discord.Embed(
            ...
            description=(
                ...
                "🚫 Do not tier players outside of their ticket\n"
                "✅ Always remain professional and respectful during tests\n"
                "✅ Only assign tiers you are confident in\n\n"
                "━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    @discord.ui.button(label="Apply — Staff", style=discord.ButtonStyle.secondary, custom_id="app_open_staff", emoji="🛡️")
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    # Unwrap CheckFailure — permission checks already send their own message
    ...
    msg = "❌ Something went wrong running that command. Please try again or contact an admin."
    try:
...
async def tier(
    ...
    # Defer immediately — DB writes + role changes + DM can exceed Discord's 3s window
    await interaction.response.defer(ephemeral=True)

    # Admins can tier anywhere. Testers must be inside a ticket channel for this user.
    is_admin = interaction.user.guild_permissions.administrator
    if not is_admin:
        with get_db() as conn:
            session = conn.execute(
                "SELECT user_id FROM sessions WHERE channel_id = ? AND status = 'pending'",
                (interaction.channel.id,)
            ).fetchone()
        if not session:
            await interaction.followup.send(
                "❌ You can only use `/tier` inside an active ticket channel.", ephemeral=True
            )
            return
        if str(session["user_id"]) != str(user.id):
            await interaction.followup.send(
                f"❌ This ticket belongs to a different player. You can only tier the player whose ticket you're in.", ephemeral=True
            )
            return
    ...
    await ensure_user(str(interaction.user.id), interaction.user.name)
    await insert_tier_record(str(user.id), gm, tr, str(interaction.user.id))

    channel = interaction.guild.get_channel(RESULT_CHANNELS.get(gm))

    tier_colors = {
        "HT1": 0xFFD700, "LT1": 0xFFD700,
        "HT2": 0xFF8C00, "LT2": 0xFF8C00,
        "HT3": 0x00C853, "LT3": 0x00C853,
        "HT4": 0x2196F3, "LT4": 0x2196F3,
        "HT5": 0x9E9E9E, "LT5": 0x9E9E9E,
    }
    embed = discord.Embed(
        title=f"⚔️ Tier Result — {gm.upper()}",
        color=tier_colors.get(tr, 0x2B2D31)
    )
    embed.add_field(name="Player", value=user.mention, inline=True)
    embed.add_field(name="Tier", value=f"**{tr}**", inline=True)
    embed.add_field(name="Tested by", value=interaction.user.mention, inline=True)
    embed.set_thumbnail(url=user.display_avatar.url)
    embed.set_footer(text=f"Dropper Tiers • {gm.upper()}")
    embed.timestamp = datetime.utcnow()
    ...
    await interaction.followup.send(f"✅ Tiered {user.mention} as **{tr}** in {gm.upper()}.", ephemeral=True)
...
async def untier(
    ...
    embed = discord.Embed(
        title="Tier Removed",
        description=f"{user.mention} was untiered",
        color=discord.Color.red()
    )
...
async def matchup(
    ...
    with get_db() as conn:
        def get_tier_db(member):
            row = conn.execute(
                """
                SELECT th.tier FROM tier_history th
                WHERE th.user_id = ? AND th.gamemode = ?
                  AND th.id = (
        ...
        tier1 = get_tier_db(user1)
        tier2 = get_tier_db(user2)

    display1 = tier1 or "Unranked"
    display2 = tier2 or "Unranked"
    rank1 = TIER_RANK.get(tier1, -1) if tier1 else -1
    rank2 = TIER_RANK.get(tier2, -1) if tier2 else -1

    if rank1 > rank2:
        result = f"{user1.mention} is higher tier"
    elif rank2 > rank1:
        result = f"{user2.mention} is higher tier"
    elif tier1 is None and tier2 is None:
        result = "Neither player is ranked"
    else:
        result = "Both players are the same tier"

    embed = discord.Embed(title=f"Matchup — {gm.upper()}", color=discord.Color.gold())
    embed.add_field(name=user1.display_name, value=display1, inline=True)
    embed.add_field(name="vs", value="\u200b", inline=True)
    embed.add_field(name=user2.display_name, value=display2, inline=True)
...
async def history(
    ...
    with get_db() as conn:
        rows = conn.execute(
            """
            SELECT th.gamemode, th.tier, th.timestamp, u.username as tester_name
    ...
    if not rows:
        ...
        for row in rows:
            ...
            lines.append(f"`{ts}` **{row['tier']}** in {row['gamemode'].upper()} — *by {tester}*")
        embed.description = "\n".join(lines)
...
async def stats(
    ...
    tiers = {r["gamemode"]: (r["tier"], r["timestamp"]) for r in rows}
    ...
    for gm in GAMEMODES:
        if gm in tiers:
            tier, ts = tiers[gm]
            lines.append(f"**{gm.upper()}** — {tier} *(tested {ts[:10]})*")
...
async def sendapplication(interaction: discord.Interaction, channel: discord.TextChannel):
    if not interaction.user.guild_permissions.administrator:
        ...
        return
    embed = discord.Embed(
        title="🏆  Join the Dropper Tiers Team",
        description=(
            "Think you have what it takes? Apply for a role on the team below.\n\n"
            "⚔️ **Tester**\n"
            "Evaluate players in 1v1s and assign official tiers across gamemodes. "
            "You must be knowledgeable, fair, and consistent.\n\n"
            "🛡️ **Staff**\n"
            "Help moderate the community, manage tickets, and keep things running smoothly. "
            "We're looking for active, trustworthy members.\n\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "*Applications are reviewed by the admin team. "
            "False information or rule violations result in an **instant ban**.*"
        ),
        color=0x7B2FBE
    )
    embed.set_image(url="https://i.imgur.com/MV0Xqr5.png")
    embed.set_footer(text="Dropper Tiers • Applications")
...
@tree.command(name="profile", description="Show a player's full tier profile")
...
async def profile(interaction: discord.Interaction, user: discord.Member = None):
    ...
    with get_db() as conn:
        ...
        link = conn.execute(
            "SELECT mc_username FROM minecraft_links WHERE discord_id = ?",
            (str(target.id),)
        ).fetchone()
        test_count = conn.execute(
            "SELECT COUNT(*) as c FROM tier_history WHERE user_id = ?",
            (str(target.id),)
        ).fetchone()["c"]

    best = max(tiers, key=lambda r: TIER_RANK.get(r["tier"], -1), default=None) if tiers else None
    ...
    embed = discord.Embed(
        ...
        color=tier_colors.get(best_tier, 0x2B2D31)
    )
    embed.set_thumbnail(url=target.display_avatar.url)

    if link:
        embed.add_field(name="Minecraft IGN", value=f"`{link['mc_username']}`", inline=True)
    embed.add_field(name="Total Tests", value=str(test_count), inline=True)
    if best_tier:
        embed.add_field(name="Best Tier", value=f"**{best_tier}** ({best['gamemode'].upper()})", inline=True)

    if tiers:
        lines = [f"**{r['gamemode'].upper()}** — {r['tier']} *(tested {r['timestamp'][:10]})*" for r in tiers]
        embed.add_field(name="Current Tiers", value="\n".join(lines), inline=False)
    else:
        embed.add_field(name="Current Tiers", value="No tiers yet.", inline=False)

    embed.set_footer(text="Dropper Tiers")
    embed.timestamp = datetime.utcnow()
    await interaction.response.send_message(embed=embed)


@tree.command(name="sendpanel", description="Send the tier test panel to a channel")
@app_commands.describe(channel="Channel to send the panel to (defaults to configured panel channel)")
async def sendpanel(interaction: discord.Interaction, channel: discord.TextChannel = None):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("You need Administrator permission to use this.", ephemeral=True)
        return
    channel = channel or interaction.guild.get_channel(PANEL_CHANNEL_ID)
    if not channel:
        await interaction.response.send_message("Panel channel not found. Specify a channel or set PANEL_CHANNEL_ID.", ephemeral=True)
        return

    embed = discord.Embed(
        title="⚔️  Tier Testing",
        description=(
            "Get your skill officially ranked by one of our certified testers.\n\n"
            "**How it works:**\n"
            "1️⃣  Click **Open Ticket** below\n"
            "2️⃣  Select your gamemode & current tier\n"
            "3️⃣  Fill in your IGN and preferred server\n"
            "4️⃣  A tester will join your ticket shortly\n\n"
            "**Available Gamemodes:**\n"
            "`Sword` • `Axe` • `Mace` • `UHC` • `Pot`\n"
            "`Crystal` • `NetheriteOP` • `SMP` • `Cart PvP`\n\n"
            "⏳ Cooldown: **30 days** per gamemode after being tiered."
        ),
        color=0x2B2D31
    )
    embed.set_image(url="https://i.imgur.com/MV0Xqr5.png")
    embed.set_footer(text="Open a ticket to get started • Testers only assign verified tiers")
...
@tree.command(name="sendsupportpanel", description="Send the support ticket panel to a channel")
@support_only()
@app_commands.describe(channel="Channel to send the panel to (defaults to the current channel)")
async def sendsupportpanel(interaction: discord.Interaction, channel: discord.TextChannel = None):
    channel = channel or interaction.channel

    embed = discord.Embed(
        title="🎫  Support",
        description=(
            "Need help or have a question? Open a support ticket below and our "
            "support team will get to you shortly.\n\n"
            "**How it works:**\n"
            "1️⃣  Click **Create Support Ticket** below\n"
            "2️⃣  Tell us why you're opening a ticket\n"
            "3️⃣  A member of support will join your ticket shortly"
        ),
        color=0x2B2D31
    )
    embed.set_image(url="https://i.imgur.com/MV0Xqr5.png")
    embed.set_footer(text="Open a ticket to get started")

    await channel.send(embed=embed, view=SupportPanelView())
    await interaction.response.send_message(f"✅ Support panel sent to {channel.mention}!", ephemeral=True)

async def validate_mc_username(username: str):
    """
    Check Mojang's API to confirm an IGN exists and get its correct capitalisation.
    Returns (True, correct_ign) on success, (False, error_message) on failure.
    Handles every failure path: 404 (not found), non-200, timeout, network error,
    and bad JSON — so callers never need to try/except this.
    """
    try:
        import aiohttp
            ...
            async with session.get(url) as resp:
                if resp.status == 200:
                    try:
                        ...
                        return True, data["name"]  # Mojang returns the correctly-cased name
                    except (KeyError, Exception):
                        # Malformed response — treat as unknown error, don't block the user
                        return True, username
                elif resp.status == 404:
                    return False, f"❌ `{username}` doesn't exist on Mojang. Check for typos."
                elif resp.status == 429:
                    # Rate limited — don't block the user, just let it through
                    return True, username
                else:
                    # Any other unexpected status — don't block the user
                    return True, username
    except asyncio.TimeoutError:
        # Mojang API slow/down — don't block the user
        return True, username
    except Exception:
        # Network error or anything else — don't block the user
        return True, username
...
async def link(interaction: discord.Interaction, minecraft_username: str):
    ...
    # Validate IGN with Mojang — fixes capitalisation, blocks nonexistent accounts
    valid, result = await validate_mc_username(minecraft_username)
    if not valid:
        await interaction.followup.send(result, ephemeral=True)
        return
    minecraft_username = result  # Use Mojang's correctly-cased version
...
@tree.command(name="forcelink", description="Manually link a Minecraft account to a Discord user (admin only)")
@app_commands.describe(user="The Discord user to link", minecraft_username="Their Minecraft IGN (case-sensitive)")
async def forcelink(interaction: discord.Interaction, user: discord.Member, minecraft_username: str):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("You need Administrator permission to use this.", ephemeral=True)
        return
    ...
    # Validate with Mojang — fixes capitalisation, blocks nonexistent accounts
    valid, result = await validate_mc_username(minecraft_username)
    if not valid:
        await interaction.followup.send(result, ephemeral=True)
        return
    minecraft_username = result
    ...
    with get_db() as conn:
        ...
        if taken and taken["discord_id"] != discord_id:
            other = interaction.guild.get_member(int(taken["discord_id"]))
            other_mention = other.mention if other else f"`{taken['discord_id']}`"
            await interaction.followup.send(
                f"❌ `{minecraft_username}` is already linked to {other_mention}. "
                f"Use `/forceunlink` on that user first, then try again.",
                ephemeral=True
            )
            return
    ...
    verb = "updated" if existing else "linked"
    old = f" (was `{existing['mc_username']}`)" if existing else ""
    await interaction.followup.send(
        f"✅ Successfully {verb} {user.mention}'s Minecraft account to `{minecraft_username}`{old}.",
        ephemeral=True
    )


@tree.command(name="forceunlink", description="Remove a user's linked Minecraft account (admin only)")
...
async def forceunlink(interaction: discord.Interaction, user: discord.Member):
    ...
    with get_db() as conn:
        ...
        if not row:
            await interaction.response.send_message(
                f"{user.mention} doesn't have a Minecraft account linked.", ephemeral=True
            )
            return
    ...
    await interaction.response.send_message(
        f"✅ Unlinked `{row['mc_username']}` from {user.mention}.", ephemeral=True
    )
