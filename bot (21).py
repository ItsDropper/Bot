...
class _ConnWrapper:
    ...
    def execute(self, sql, params=()):
        # Postgres uses %s placeholders; convert SQLite's ? placeholders
...
def get_db():
    global _turso_conn
    try:
        if _turso_conn is None or _turso_conn.closed:
            raise Exception("reconnect")
        # Ping to detect stale connection
        _turso_conn.cursor().execute("SELECT 1")
    except Exception:
        _turso_conn = psycopg2.connect(
            os.environ["DATABASE_URL"],
            sslmode="require"
        )
        _turso_conn.autocommit = False
    return _ConnWrapper(_turso_conn)
