...
async def backup_db_to_github():
    ...
    if not GITHUB_TOKEN:
        print("GitHub backup aborted: GITHUB_TOKEN is not set in the environment.")
        return
    try:
        import base64
        import aiohttp
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.get(url, headers=headers) as resp:
                ...
                if resp.status == 200:
                    ...
                    print(f"GitHub backup: unexpected GET status {resp.status}")
            ...
            async with session.put(url, headers=headers, json=payload) as resp:
                if resp.status in (200, 201):
                    print("GitHub backup succeeded.")
                else:
                    print(f"GitHub backup failed: {resp.status}")
                    print(await resp.text())
    except asyncio.TimeoutError:
        print("GitHub backup error: request timed out after 20s (network/DNS issue reaching api.github.com?)")
    except asyncio.CancelledError:
        print("GitHub backup was cancelled before it finished (task got cancelled, likely by shutdown/restart).")
        raise
    except Exception as e:
        print(f"GitHub backup error: {type(e).__name__}: {e}")

async def restore_db_from_github():
    # Always restore from GitHub — never skip, because Render clones the repo
    # on every deploy which puts a stale tiers.db on disk.
    try:
        import base64
        import aiohttp
        url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/{GITHUB_FILE_PATH}"
...
def insert_tier_record(user_id: str, gamemode: str, tier: str, tester_id: str, notes: str = None):
    ...
    def _log_backup_result(task: asyncio.Task):
        if task.cancelled():
            print("GitHub backup task was cancelled (never completed).")
            return
        exc = task.exception()
        if exc:
            print(f"GitHub backup task raised an uncaught exception: {type(exc).__name__}: {exc}")

    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            task = loop.create_task(backup_db_to_github())
